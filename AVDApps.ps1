# --------------------------------------------------------------
# AVD – Rush Intune enrollment (Hybrid-joined hosts)
# --------------------------------------------------------------
...
# --------------------------------------------------------------
# 1. Wait for internet (only Intune endpoints)
# --------------------------------------------------------------
$max = 300   # 5 min
...
if(-not $ok){ Log "FATAL – no internet after $max s"; exit 1 }

# --------------------------------------------------------------
# 2. Force Hybrid Entra ID join (dsregcmd)
# --------------------------------------------------------------
...
if(-not $joined){
    Log "Hybrid join missing – forcing..."
    & dsregcmd.exe /join /debug | Out-Null
    $t = [Diagnostics.Stopwatch]::StartNew()
    while($t.Elapsed.TotalSeconds -lt 180){
        $s = & dsregcmd.exe /status 2>$null
        if($s -match 'AzureAdJoined\s*:\s*YES'){ Log "Hybrid join SUCCESS"; break }
    ...
    if(-not (& dsregcmd.exe /status 2>$null) -match 'AzureAdJoined\s*:\s*YES'){
        Log "WARN – join did NOT complete, continuing anyway"
    }
}else{ Log "Already hybrid joined" }

# --------------------------------------------------------------
# 3. Trigger MDM enrollment (Intune)
# --------------------------------------------------------------
...
try{
    ...
    Invoke-CimMethod -Namespace $ns -ClassName $cls -MethodName Enroll `
        -Arguments @{FriendlyName=$o.FriendlyName; SessionId=(New-Guid).Guid} | Out-Null
